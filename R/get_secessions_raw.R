get_secessions_raw <- function(){
    ...
    # Explore California new school districts
    years <- sort(unique(sd_shapes[STATE_NAME %in% 'california']$YEAR))
    year_lag <- years[1:(length(years)-1)]
    year_lead <- years[2:length(years)]
    dt_years <- data.table(year_lag, year_lead)
    l <- vector('list', nrow(dt_years))
    for(i_year in 1:nrow(dt_years)){
      sd_base_year <- sd_shapes[STATE_NAME %in% 'california' & YEAR %in% dt_years[i_year]$year_lag]$LEAID
      sd_new_year <- sd_shapes[STATE_NAME %in% 'california' & YEAR %in% dt_years[i_year]$year_lead]$LEAID
      l_sds <- list(base_year = dt_years[i_year]$year_lag,
                     current_year = dt_years[i_year]$year_lead,
                     sd_new = sd_new_year[!(sd_new_year %in% sd_base_year)],
                     sd_leave = sd_base_year[!(sd_base_year %in% sd_new_year)])
      l[[i_year]] <- l_sds
    }
    str(l)
