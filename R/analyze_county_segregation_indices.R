analyze_county_segregation_indices <- function(){
  analyze_county_segregation_indices_location <- '~/Dropbox/pkg.data/nclb_segregation/Clean/analyze_county_segregation_indices.rds'
  if(!file.exists(analyze_county_segregation_indices_location)){
    l_schools <- get_schools()
    dt_inds <- create_segregation_indices_county()
    dt_inds <- dt_inds[!is.na(dissimilarity) & !is.na(exposure_bw) & !is.na(exposure_wb)]
    
    dt_inds_agg <- unique(dt_inds[, .(dissimilarity=mean(dissimilarity),
                                  exposure_bw=mean(exposure_bw),
                                  exposure_wb=mean(exposure_wb)), by=YEAR])
    dt_inds_agg <- data.table(tidyr::gather(dt_inds_agg, key=measure, value=value , 2:4))
    dt_inds_agg <- dt_inds_agg[,YEAR:=as.integer(YEAR)]
    
    ggplot2::ggplot(dt_inds_agg, aes(x=YEAR, y=value, group=measure, color=measure)) +
      geom_line() + 
      ggtitle('School Segregation Measures by County')
      ggsave('~/Dropbox/pkg.data/nclb_segregation/plots/county_level_segregation_mean.pdf')
    ...
    l_out$boundaries_lifted_dissanalyze_segregation_indices <- function(){
    ...
    imilarity <- ggplot2::ggplot(dt_plots, aes(x=YEAR, y=dissimilarity, group=boundaries_lifted, color=boundaries_lifted)) + 
      geom_line() + 
      ggtitle('Dissimilarity')
    print(l_out$boundaries_lifted_dissimilarity)
